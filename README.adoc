...
link:src/test/munit/option-a/examples/parameterized/README.adoc[*The HTTP status → Mule error type map*] answers that for 58 status codes, with every row backed by a green MUnit test built on Option A. (The four 1xx codes and 408 are currently excluded from the data — the map says why.) The headline finding:
